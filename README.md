...
ใส่ข้อมูลตั้งต้น (ผู้ใช้งาน · หมวดหมู่ · ประกาศ · กฎอัตโนมัติ · ฐานความรู้ — **คิวงานเริ่มต้นว่าง**):
...
- *แดชบอร์ดภาพรวม* — ตั๋วงานเปิดอยู่, อัตราแก้ปัญหาสำเร็จ, เวลาตอบกลับเฉลี่ย, จำนวนที่เกิน SLA, กราฟปริมาณตั๋ว 14 วัน (แจ้งเข้า vs ปิดงาน), สัดส่วนตามหมวดหมู่, รายการตั๋วที่เสี่ยงเกิน SLA
- *ตั้งค่าระบบ* — จัดการหมวดหมู่ปัญหา (ชื่อ สี SLA กลุ่มงาน), ตารางระดับความรุนแรงและเวลาตอบสนอง, เปิด/ปิดกฎมอบหมายอัตโนมัติและกฎ Escalation
...
- กำหนดระดับความสำคัญ Low / Medium / High / Critical — ระบบคำนวณกำหนดเสร็จตาม SLA ใหม่ทันที
...
- **กฎมอบหมายอัตโนมัติ (ASSIGN)** — เช่น หมวดหมู่ = Network → มอบหมายให้ทีมเครือข่าย
- **กฎยกระดับ (ESCALATE)** — เบื้องหลังมี job ตรวจทุก 5 นาที แจ้งเตือนเมื่อเหลือ SLA ต่ำกว่าเกณฑ์หรือค้างเกินเวลา (กันแจ้งซ้ำภายใน 60 นาที)
- **SLA** — คำนวณจากระดับความรุนแรง: Critical 1 ชม. · High 4 ชม. · Medium 1 วัน · Low 3 วัน
...
│   │   ├── jobs/               ตรวจ SLA และ Escalation
│   │   └── utils/              SLA · เลขตั๋ว · แจ้งเตือน · กฎอัตโนมัติ
...
| GET · POST | `/api/tickets` | ล็อกอิน | รายการ / สร้างตั๋ว (multipart รองรับไฟล์แนบ) |
...
| GET · POST · PATCH · DELETE | `/api/rules` | admin | กฎอัตโนมัติ |
...
**ตรวจว่าแชทเรียลไทม์ทำงานจริง** — เปิด `https://<โดเมนของคุณ>/api/health` ขณะมีคนใช้งานอยู่
ค่า `sockets` ต้องมากกว่า 0 ถ้าเป็น 0 ตลอดแปลว่า WebSocket ต่อไม่ติด

```json
{ "ok": true, "service": "itsm-api", "sockets": 2 }
```
...
เมื่อต้องการเคลียร์คิวงานให้ว่างอีกครั้ง (เช่น หลังทดลองใช้งาน):

```bash
npm run clear:tickets
```

- **ลบ:** ตั๋วงาน · ข้อความแชทในตั๋ว · การแจ้งเตือนที่ผูกกับตั๋ว · ตัวนับเลขตั๋ว (เลขจะเริ่มใหม่จาก `000001`)
- **เก็บไว้:** ผู้ใช้งาน · หมวดหมู่ · ประกาศ · กฎอัตโนมัติ · ฐานความรู้

> คำสั่งนี้ทำงานกับ MongoDB ที่ระบุใน `MONGODB_URI` — **ถ้าใช้โหมดสาธิต** (`npm run dev:demo`) ฐานข้อมูลอยู่ในหน่วยความจำ ให้ปิดแล้วเปิดใหม่แทน ระบบจะเริ่มด้วยคิวงานว่างเสมอ
